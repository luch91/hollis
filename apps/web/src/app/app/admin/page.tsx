...
async function inviteMember(formData: FormData) {
  ...
  (await cookies()).set("hollis_invitation_token", invitation.token, { httpOnly: true, maxAge: 5 * 60, path: "/", sameSite: "strict", secure: process.env.NODE_ENV === "production" });
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ invite?: string }> }) {
  const [profile, members, invitations, auditEvents, params] = await Promise.all([api("/v1/workspace"), api("/v1/workspace/members"), api("/v1/workspace/invitations"), api("/v1/workspace/audit-events"), searchParams]);
  return <section className="content-panel admin-workspace">
    ...
    {params.invite === "ready" ? <section className="admin-card invitation-link"><h2>Invitation link</h2><InvitationLink /></section> : null}
    ...
    <section className="admin-card"><h2>Control history</h2><p>Workspace changes are recorded in an append-only hash chain.</p><div className="admin-list">{auditEvents.length ? auditEvents.map((event: any) => <div className="admin-row" key={`${event.eventSequence}-${event.eventHash}`}><div><strong>{event.eventType.replaceAll("_", " ")}</strong><small>{new Date(event.createdAt).toLocaleString()} · {event.eventHash.slice(0, 20)}…</small></div><small>#{event.eventSequence}</small></div>) : <p>No workspace control events yet.</p>}</div></section>
  </section>;
}

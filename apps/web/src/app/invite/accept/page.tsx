...
export default async function AcceptInvitation({
  ...
  if (!session)
    redirect(`/sign-in?returnTo=${encodeURIComponent(`/invite/accept?token=${token ?? ""}`)}`);
  async function accept() {
    "use server";
    if (!token) redirect("/onboarding");
    const response = await fetch(`${apiUrl}/v1/workspace-invitations/accept`, {
      method: "POST",
      headers: { authorization: `Bearer ${session}`, "content-type": "application/json" },
      body: JSON.stringify({ token }),
    });
    if (!response.ok) redirect("/invite/accept?error=unavailable");
    redirect("/app");
  }
  return (
    <main className="auth-shell">
      <section className="auth-card">
        ...
        <p className="auth-summary">
          Your verified email must match the recipient of this invitation.
        </p>
        {error ? (

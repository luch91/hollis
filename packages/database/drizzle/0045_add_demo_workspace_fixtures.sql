CREATE TABLE "demo_workspace_fixtures" (
...
CREATE INDEX "demo_workspace_fixtures_expiry_idx" ON "demo_workspace_fixtures" (expires_at);
ALTER TABLE "demo_workspace_fixtures" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "demo_workspace_fixtures" FORCE ROW LEVEL SECURITY;
CREATE POLICY "demo_workspace_fixtures_tenant_isolation" ON "demo_workspace_fixtures"
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.demo_workspace_fixtures WHERE tenant_id = input_tenant_id AND expires_at > now()) THEN
    RETURN 0;
  END IF;
  ...
  FOREACH case_id IN ARRAY ARRAY[gen_random_uuid(), gen_random_uuid(), gen_random_uuid()] LOOP
    ...
    evidence_digest := 'sha256:' || encode(digest(convert_to(case_id::text, 'utf8'), 'sha256'), 'hex');
    ...
    VALUES (input_actor_id::text, case_id, 'sha256:' || encode(digest(convert_to(case_id::text || expires_at::text, 'utf8'), 'sha256'), 'hex'), 'case_created', jsonb_build_object('demo', true, 'policyId', 'asterion-demo-governance-2026', 'ruleId', 'human-review-required'), null, input_tenant_id);
...
BEGIN
  ...
  FROM public.demo_workspace_fixtures
  WHERE tenant_id = input_tenant_id AND expires_at <= now();
  DELETE FROM public.review_cases
  WHERE tenant_id = input_tenant_id
    AND id IN (SELECT case_id FROM public.demo_workspace_fixtures WHERE tenant_id = input_tenant_id AND expires_at <= now());
  DELETE FROM public.policy_versions
  WHERE tenant_id = input_tenant_id
    AND id IN (SELECT policy_version_id FROM public.demo_workspace_fixtures WHERE tenant_id = input_tenant_id AND expires_at <= now());
  DELETE FROM public.demo_workspace_fixtures
  WHERE tenant_id = input_tenant_id AND expires_at <= now();

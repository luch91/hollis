...
export async function POST(request: Request) {
  ...
  if (!secret || !supplied || process.env.VERCEL_ENV !== "preview") return new NextResponse(null, { status: 404 });
  ...
  const response = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${apiKey}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, password, returnSecureToken: true }), cache: "no-store" });
  const payload = (await response.json().catch(() => null)) as { idToken?: unknown } | null;
  if (!response.ok || typeof payload?.idToken !== "string") return new NextResponse(null, { status: 401 });

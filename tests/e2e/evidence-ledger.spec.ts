...
test("attachments are case-scoped, idempotent, ordered, and frozen at claim", async ({
  page,
}, testInfo) => {
  ...
  expect(
    (
      await page.request.put(retryRecord.uploadUrl, {
        data: bytes[0],
        headers: { "content-type": "text/plain" },
      })
    ).status(),
  ...
  expect(
    (
      await page.request.post(
        `${apiOrigin}/v1/review-cases/${caseId}/evidence/${retryRecord.evidenceId}/verify`,
        { headers: { authorization } },
      )
    ).status(),
  ).toBe(204);
  const afterRetry = await page.request.get(`${apiOrigin}/v1/review-cases/${caseId}`, {
    headers: { authorization },
  });
  expect((await afterRetry.json()).evidence.map((item: { digest: string }) => item.digest)).toEqual(
    bytes.map(hash),
  );

  const independentCaseId =
    testInfo.project.name === "chromium-mobile"
      ? "00000000-0000-4000-8000-000000000410"
      : "00000000-0000-4000-8000-000000000411";
  const independentUpload = await page.request.post(
    `${apiOrigin}/v1/review-cases/${independentCaseId}/evidence/uploads`,
    {
      data: { digest: hash(bytes[0]), mediaType: "text/plain", sizeBytes: bytes[0].byteLength },
      headers: { authorization },
    },
  ...
  const independentRecord = (await independentUpload.json()) as {
    evidenceId: string;
    uploadUrl: string;
  };
  expect(
    (
      await page.request.put(independentRecord.uploadUrl, {
        data: bytes[0],
        headers: { "content-type": "text/plain" },
      })
    ).status(),
  ...
  expect(
    (
      await page.request.post(
        `${apiOrigin}/v1/review-cases/${independentCaseId}/evidence/${independentRecord.evidenceId}/verify`,
        { headers: { authorization } },
      )
    ).status(),
  ).toBe(204);
  const independentDetail = await page.request.get(
    `${apiOrigin}/v1/review-cases/${independentCaseId}`,
    { headers: { authorization } },
  );
  expect(
    (await independentDetail.json()).evidence.some(
      (item: { digest: string }) => item.digest === hash(bytes[0]),
    ),
  ).toBe(true);

  expect(
    (
      await page.request.post(`${apiOrigin}/v1/review-cases/${caseId}/claim`, {
        headers: { authorization },
      })
    ).status(),
  ).toBe(200);
  expect(
    (
      await page.request.delete(
        `${apiOrigin}/v1/review-cases/${caseId}/evidence/${body.evidence[0]?.id}`,
        { headers: { authorization } },
      )
    ).status(),
  ).toBe(409);
  const afterFreeze = await page.request.get(`${apiOrigin}/v1/review-cases/${caseId}`, {
    headers: { authorization },
  });
  expect(
    (await afterFreeze.json()).evidence.map((item: { digest: string }) => item.digest),
  ).toEqual(bytes.map(hash));
});

...
if (!baseURL || !allowedOrigin || !identityToken || !approval || !reviewCaseId || !workspaceName) {
  throw new Error(
    "The real workflow requires its web origin, exact allowed origin, identity token, approved environment class, review case ID, and workspace name.",
  );
}

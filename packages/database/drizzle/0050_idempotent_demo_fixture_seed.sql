...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.demo_workspace_fixtures AS d
    WHERE d.tenant_id = input_tenant_id AND d.expires_at > now()
  ) THEN RETURN 0; END IF;

  SELECT pv.id INTO policy_id FROM public.policy_versions AS pv
  WHERE pv.tenant_id = input_tenant_id AND pv.policy_id = 'asterion-demo-governance-2026' AND pv.version = '1.0';
  ...
  ON CONFLICT (policy_version_id, control_id) DO NOTHING;

  FOREACH case_id IN ARRAY ARRAY[gen_random_uuid(), gen_random_uuid(), gen_random_uuid()] LOOP
    stage := stage + 1; evidence_id := gen_random_uuid();
    ...
    VALUES (CASE WHEN stage > 1 THEN input_actor_id::text ELSE null END, 'asterion-demo-gate-1.0', CASE WHEN stage = 3 THEN 'approved' ELSE null END, CASE WHEN stage = 3 THEN 'Synthetic demo decision recorded for workflow demonstration.' ELSE null END, CASE WHEN stage = 3 THEN now() ELSE null END, CASE WHEN stage = 3 THEN input_actor_id::text ELSE null END, jsonb_build_array(jsonb_build_object('digest', evidence_digest, 'id', evidence_id, 'mediaType', 'text/plain')), 'DEMO-ASTERION-' || substr(case_id::text, 1, 8), CASE WHEN stage = 3 THEN 'approve' ELSE null END, case_id, evidence_digest, 'asterion-demo-governance-2026', '1.0', 'refer', CASE WHEN stage = 3 THEN 'medium' ELSE 'high' END, now() + interval '7 days', 'human-review-required', stage_status, input_tenant_id);
    ...
    END IF;
    INSERT INTO public.demo_workspace_fixtures (case_id, expires_at, policy_version_id, tenant_id) VALUES (case_id, expires_at, policy_id, input_tenant_id);

...
BEGIN
  ...
  RETURN QUERY SELECT resolved_session_id, resolved_user_id, resolved_tenant_id, resolved_workspace_name, resolved_role;
END;
$$;
...
BEGIN
  ...
  SELECT session.id, session.user_id INTO resolved_session_id, resolved_user_id
  FROM public.application_sessions AS session
  WHERE session.token_digest = input_token_digest
    AND session.revoked_at IS NULL
    AND session.expires_at > now()
  ...
  RETURN QUERY SELECT resolved_session_id, resolved_user_id, input_tenant_id, resolved_workspace_name, resolved_role;
END;
$$;

...
export function InvitationLink() {
  ...
  return <><p>Copy this link now. It will not be stored or emailed by Hollis.</p><div className="invitation-link-control"><input aria-label="Invitation link" readOnly value={invitationLink} /><button className="text-button" onClick={() => void copy()} type="button">{copied ? "Copied" : "Copy link"}</button></div>{copyError ? <p className="auth-error">{copyError}</p> : null}</>;
}

...
async function switchWorkspaceAction(formData: FormData) {
  ...
  const response = await fetch(`${apiUrl}/v1/auth/active-workspace`, {
    method: "POST",
    ...
    <details className="workspace-switcher">
      ...
      <div>
        {workspaces.map((workspace) =>
          workspace.tenantId === activeWorkspaceId ? (
            <span key={workspace.tenantId}>
              {workspace.workspaceName}
              <small>Current workspace</small>
            </span>
          ) : (
            <form action={switchWorkspaceAction} key={workspace.tenantId}>
              <input name="tenantId" type="hidden" value={workspace.tenantId} />
              <button type="submit">
                {workspace.workspaceName}
